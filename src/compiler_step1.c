...
Type* compileEnum(Compiler* c)
{
	...
	while (1)
	{
		Def* def;
		LINT pIndex;
		if ((!parserNext(c))||((!islabel(c->parser->token))&&strcmp(c->parser->token, ";;"))) return compileError(c,"label or ';;' expected (found %s)\n", compileToken(c));
		...
		def = pkgAddConstInt(c->th, c->pkg, c->parser->token, num++, MM.Int); if (!def) return NULL;
		defSetParser(def, c, pIndex);
	}
}
Type* compileExport(Compiler* c)
{
	...
	while (parserNext(c)) {
		if (!islabel(c->parser->token)) {
			if (count) {
				STACK_PUSH_NIL_ERR(c->th, NULL);
				while(count) {
					STACK_PUSH_FILLED_ARRAY_ERR(c->th, LIST_LENGTH, DBG_LIST, NULL);
					count--;
				}
				STACK_PUSH_PNT_ERR(c->th, c->exports, NULL);
				STACK_PUSH_FILLED_ARRAY_ERR(c->th, LIST_LENGTH, DBG_LIST, NULL);
				c->exports = STACK_PULL_PNT(c->th);
			}
			if (!strcmp(c->parser->token, ";;")) return MM.Boolean;
			if (strcmp(c->parser->token, ",")) return compileError(c,"';;' or ';' or label expected (found %s)\n", compileToken(c));
		}
		else {
			...
			STACK_PUSH_STR_ERR(c->th, c->parser->token, -1, NULL);
			count++;
		}
	}
	return NULL;
