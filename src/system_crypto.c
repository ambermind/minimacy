...
int fun_md5Process(Thread* th)
{
	...
	LSmd5* md5 = (LSmd5*)STACKPNT(th, 3);
	if (!md5) FUN_RETURN_NIL;
	FUN_SUBSTR(src,start,len,lenIsNil,STRLEN(src));

	MD5Update(&md5->context, (unsigned char*)STRSTART(src) + start, (int)len);
	FUN_RETURN_PNT((LB*)md5);
}
...
{
	LB* buffer;
	LSsha1* sha = (LSsha1*)STACKPNT(th, 0);
	if (!sha) FUN_RETURN_NIL;

	buffer = memoryAllocStr(th,NULL, 20); if (!buffer) return EXEC_OM;
	SHAFinal(&sha->context,(unsigned char*)STRSTART(buffer));
	FUN_RETURN_PNT(buffer);
}
int fun_sha1Process(Thread* th)
{
	...
	LINT start = STACKINT(th, 1);
	LB* src = STACKPNT(th, 2);
	LSsha1* sha = (LSsha1*)STACKPNT(th, 3);
	if (!sha) FUN_RETURN_NIL;
	FUN_SUBSTR(src, start,len,lenIsNil,STRLEN(src));
	SHAUpdate(&sha->context, (unsigned char*)STRSTART(src) + start, (int)len);
	FUN_RETURN_PNT((LB*)sha);
}
...
{
	LB* buffer;
	LSsha256* sha=(LSsha256*)STACKPNT(th,0);
	if (!sha) FUN_RETURN_NIL;
...
int fun_sha256Process(Thread* th)
{
	...
	LINT start=STACKINT(th,1);
	LB* src=STACKPNT(th,2);
	LSsha256* sha=(LSsha256*)STACKPNT(th,3);
	if (!sha) FUN_RETURN_NIL;
	FUN_SUBSTR(src,start,len,lenIsNil,STRLEN(src));
	sha256process(&sha->context,STRSTART(src)+start,(int)len);
	FUN_RETURN_PNT((LB*)sha);
}
...
{
	LB* buffer;
	LSsha384* sha = (LSsha384*)STACKPNT(th, 0);
	if (!sha) FUN_RETURN_NIL;
...
int fun_sha384Process(Thread* th)
{
	...
	LINT start = STACKINT(th, 1);
	LB* src = STACKPNT(th, 2);
	LSsha384* sha = (LSsha384*)STACKPNT(th, 3);
	if (!sha) FUN_RETURN_NIL;
	FUN_SUBSTR(src, start, len, lenIsNil, STRLEN(src));
	sha384process(&sha->context, STRSTART(src) + start, (int)len);
	FUN_RETURN_PNT((LB*)sha);
}
...
{
	LB* buffer;
	LSsha512* sha=(LSsha512*)STACKPNT(th,0);
	if (!sha) FUN_RETURN_NIL;
...
int fun_sha512Process(Thread* th)
{
	...
	LINT start=STACKINT(th,1);
	LB* src=STACKPNT(th,2);
	LSsha512* sha=(LSsha512*)STACKPNT(th,3);
	if (!sha) FUN_RETURN_NIL;
	FUN_SUBSTR(src, start, len, lenIsNil, STRLEN(src));
	sha512process(&sha->context,STRSTART(src)+start,(int)len);
	FUN_RETURN_PNT((LB*)sha);
}
...
int fun_aesEncrypt(Thread* th)
{
	...
	LScbc* d=(LScbc*)STACKPNT(th,2);
	if (!d) FUN_RETURN_NIL;
	FUN_CHECK_CONTAINS(src, index, AES_BLOCKLEN, STRLEN(src));
...
{
	LINT index = STACKINT(th, 0);
	LB* dst = STACKPNT(th, 1);
	LScbc* d = (LScbc*)STACKPNT(th, 2);
	if (!d) FUN_RETURN_NIL;
	FUN_CHECK_CONTAINS(dst, index, AES_BLOCKLEN, STRLEN(dst));
...
int fun_desEncrypt(Thread* th)
{
	LINT index = STACKINT(th, 0);
	LB* src = STACKPNT(th, 1);
	LSdes* d = (LSdes*)STACKPNT(th, 2);
	if (!d) FUN_RETURN_NIL;
	FUN_CHECK_CONTAINS(src, index, DES_BLOCKLEN, STRLEN(src));
...
int fun_desWriteBytes(Thread* th)
{
	...
	LSdes* d = (LSdes*)STACKPNT(th, 2);
	if (!d) FUN_RETURN_NIL;
	FUN_CHECK_CONTAINS(dst, index, DES_BLOCKLEN, STRLEN(dst));
...
int fun_rc4Output(Thread* th)
{
	...
	LINT start = STACKINT(th, 1);
	LB* src = STACKPNT(th, 2);
	LSrc4* d = (LSrc4*)STACKPNT(th, 3);
	if (!d) FUN_RETURN_NIL;
	FUN_SUBSTR(src, start,len,lenIsNil,STRLEN(src));

	buffer = memoryAllocStr(th, NULL, len); if (!buffer) return EXEC_OM;
	RC4Process(&d->context, STRSTART(src) + start, len, STRSTART(buffer));
	FUN_RETURN_PNT(buffer);
}
...
int fun_rc4WriteBytes(Thread* th)
{
	...
	LSrc4* d = (LSrc4*)STACKPNT(th, 5);
	if (!d) FUN_RETURN_NIL;
	FUN_COPY_CROP(dst, index, STRLEN(dst), src, start, len, lenIsNil, STRLEN(src));

	RC4Process(&d->context, STRSTART(src) + start, len, STRSTART(dst)+ index);
	FUN_RETURN_PNT((LB*)d);
}
...
int fun_crc32(Thread* th)
{
	...
	LINT crc = STACKINT(th, 0);
	LB* src = STACKPNT(th, 1);
	if (!src) FUN_RETURN_NIL;
	v = crc32Str((unsigned int)crc, STRSTART(src), (int)STRLEN(src));
...
int fun_adler32(Thread* th)
{
	...
	LB* src = STACKPNT(th, 1);
	if (!src) FUN_RETURN_NIL;
	v = adler32Str((unsigned int)crc, STRSTART(src), (int)STRLEN(src));
	FUN_RETURN_INT(v & 0xffffffff);
}
...
int coreCryptoInit(Thread* th, Pkg *system)
{
	...
	Type* fun_Md5_B_I_I_Md5 = typeAlloc(th, TYPECODE_FUN, NULL, 5,
		Md5->type, MM.Bytes, MM.I, MM.I, Md5->type);
	...
	Type* fun_Sha1_B_I_I_Sha1 = typeAlloc(th,TYPECODE_FUN, NULL, 5,
		Sha1->type, MM.Bytes, MM.I, MM.I, Sha1->type);
	...
	Type* fun_Sha256_B_I_I_Sha256=typeAlloc(th,TYPECODE_FUN,NULL,5,
		Sha256->type,MM.Bytes,MM.I,MM.I,Sha256->type);
	...
	Type* fun_Sha384_B_I_I_Sha384=typeAlloc(th,TYPECODE_FUN,NULL,5,
		Sha384->type,MM.Bytes,MM.I,MM.I,Sha384->type);
	...
	Type* fun_Sha512_B_I_I_Sha512=typeAlloc(th,TYPECODE_FUN,NULL,5,
		Sha512->type,MM.Bytes,MM.I,MM.I,Sha512->type);
	...
	pkgAddConstInt(th, system,"AES_BLOCK",AES_BLOCKLEN,MM.I);
	...
	pkgAddConstInt(th, system, "DES_BLOCK",DES_BLOCKLEN, MM.I);
	...
	pkgAddFun(th, system, "strCrc32", fun_crc32, typeAlloc(th, TYPECODE_FUN, NULL, 3, MM.S, MM.I, MM.I));
	pkgAddFun(th, system, "bytesCrc32", fun_crc32, typeAlloc(th, TYPECODE_FUN, NULL, 3, MM.Bytes, MM.I, MM.I));
	pkgAddFun(th, system, "strAdler32", fun_adler32, typeAlloc(th, TYPECODE_FUN, NULL, 3, MM.S, MM.I, MM.I));
	pkgAddFun(th, system, "bytesAdler32", fun_adler32, typeAlloc(th, TYPECODE_FUN, NULL, 3, MM.Bytes, MM.I, MM.I));
	return 0;
}
