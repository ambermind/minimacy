...
void parserRestoreFromRef(Compiler* c,Ref* ref)
{
	if (c->parser) parserReset(c);
	c->parser = ref->parser;
	parserJump(c,(int)ref->parserIndex);
}
...
{
	return sourceToStr(c->th, b, c->parser->token, strlen(c->parser->token));
}

int parserIsFinal(char* token)
{
	if (!token) return 1;
	if (strstr(")}]:,;; ->",token)) return 1;
	return 0;
}
...
//	printf("parserFromData %s "LSX" -> "LSX"\n", name, c, parser);
	parser->name = NULL;
	parser->block = NULL;
...
Type* parserFromIncludes(Compiler* c,char* name)
{
	...
	src = fileReadPackage(c->th, name, SUFFIX_CODE, NULL);
