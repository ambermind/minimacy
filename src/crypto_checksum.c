...
{
	int i;
	// value is expected to be 0 the first time
	value ^= 0xFFFFFFFF;	
	for (i = 0; i < len; i++) {
		unsigned int index = (value ^ src[i]) & 0xff;
		value = (value >> 8) ^ CRC32[index];
	}
	value ^= 0xFFFFFFFF;
	return value;
}

const unsigned char CRC7[256]={
...
unsigned int crc7Str(unsigned int value, char* src, int len)
{
	int i;
	// value is expected to be 0 the first time
	for (i = 0; i < len; i++) value= CRC7[255&((value<<1)^src[i])];
	return value;
}
